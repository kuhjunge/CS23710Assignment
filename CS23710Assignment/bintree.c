...
void inorder_tableprint(Competitor_ptr_t * root_ptr)
{
    if (LOG_DEBUG)
    {
        printf("DEBUG: inorder Sort (table_print)\n");
    }

    if (*root_ptr != NULL)
    {
        inorder_tableprint(&(*root_ptr) -> left);
        print_line_competitor(*root_ptr);
        inorder_tableprint(&(*root_ptr) -> right);
    }
}
...
void delete_tree(Competitor_ptr_t * root_ptr)
{
    ...
    if (*root_ptr != NULL)
    {
        ...
        if (LOG_DEBUG)
        {
            printf("DEBUG: free space of Node!\n");
        }
