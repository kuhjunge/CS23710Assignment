                      ...
                      char * linebuffer)
{
    if (f == NULL)
    {
        return NULL;    // error: file handle not initialised
    }
    ...
    else
    {
        ...
        else
        {
            if (LOG_DEBUG)
            {
                printf("DEBUG: Read from File: %s", linebuffer);
            }
        }
    }
 ...
 * @return
 */
float read_veg_or_fruit_next_line(FILE * f,
                                  char * linebuffer)
{
    int   feet = 0;
    float inch = 0;

    if (read_next_line(f, linebuffer) != NULL)
    {    // length_cucumber;
        ...
        Competitor_ptr_t *                          comp_ptr_ptr)
{
    ...
    *comp_ptr_ptr      = (Competitor_ptr_t) calloc(1, sizeof(Competitor_t));
    ...
    if (read_next_line(competition_ptr -> process_file, linebuffer) != NULL)
    {
        ...
        comp_ptr -> length_cucumber    = read_veg_or_fruit_next_line(competition_ptr -> process_file, linebuffer);
        comp_ptr -> length_carrot      = read_veg_or_fruit_next_line(competition_ptr -> process_file, linebuffer);
        comp_ptr -> length_runner_bean = read_veg_or_fruit_next_line(competition_ptr -> process_file, linebuffer);

        if ((comp_ptr -> length_runner_bean != ERR_FLOAT) && (comp_ptr -> length_carrot != ERR_FLOAT)
                && (comp_ptr -> length_cucumber != ERR_FLOAT))
        {
            if (LOG_DEBUG)
            {
                printf("DEBUG: Name: %s\nNumber: %d\nAddress: %s\nFon: %s\n"
                       "Cucumber: %f\nCarrot: %f\nRunner Bean: %f"
                       "\n---------------------------------\n", comp_ptr -> name, comp_ptr -> number, comp_ptr ->
                            address, comp_ptr -> phone_number, comp_ptr -> length_cucumber, comp_ptr ->
                             length_carrot, comp_ptr -> length_runner_bean);
            }
    ...
    if (LOG_DEBUG)
    {
        printf("ERROR: Competitor data corrupted or no competitor found!\n");
    }

    /* Remove the non existent competitor */
    free(comp_ptr);
