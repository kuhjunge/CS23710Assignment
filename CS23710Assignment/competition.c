...
float calc_total_length(Competitor_ptr_t comp_ptr){
    ...
    return comp_ptr->length_cucumber + comp_ptr->length_carrot 
            + comp_ptr->length_runner_bean;
}
...
void print_line_competitor(Competitor_ptr_t competitor){
    printf("%s\t%d\t%2dft %4.1fin   %2dft %4.1fin   %2dft %4.1fin   %2dft %4.1fin\n",
            ...
            calc_foot_length(competitor->length_cucumber),
            calc_inch_length(competitor->length_cucumber),
            calc_foot_length(competitor->length_carrot),
            calc_inch_length(competitor->length_carrot),
            calc_foot_length(competitor->length_runner_bean),
            calc_inch_length(competitor->length_runner_bean),
            calc_foot_length(calc_total_length(competitor)),
...
void print_task_one(Competition_t* competition_ptr){
    ...
    printf("Competition: %s Date: %s\n", competition_ptr->name,
            competition_ptr->date);
